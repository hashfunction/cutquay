...
import './i18n.js';
import { canOpenExternal } from '../common/navigation.js';
...
async function openExternal(url: string) {
  if (!canOpenExternal(url)) throw new Error('Unsupported external URL');
...
function createWindow() {
  ...
  // Never load web content in the renderer that retains Node integration.
  mainWindow.webContents.on('will-navigate', (event) => event.preventDefault());
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (canOpenExternal(url)) openExternal(url).catch((error: unknown) => logger.warn('Could not open URL', error));
    return { action: 'deny' };
  });
...
ipcMain.handle('__electron_rpc__', async (_event, method: keyof RemoteApi, args: any[]) => {
  assert(Object.hasOwn(remoteApi, method), `Unknown API method: ${method}`);
