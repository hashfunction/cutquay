...
import { recipeSettings, type ExportRecipeV1 } from '../../../common/exportRecipe';
...
function ExportConfirm({
  ...
  onRecipeFormatChange: (format: string) => void,
  ...
  const recipeControls = useUserSettings();
  const applyRecipe = (recipe: ExportRecipeV1) => {
    const values = recipeSettings(recipe);
    recipeControls.setAutoMerge(values.autoMerge);
    recipeControls.setAutoDeleteMergedSegments(values.autoDeleteMergedSegments);
    recipeControls.setSegmentsToChaptersOnly(values.segmentsToChaptersOnly);
    recipeControls.setKeyframeCut(values.keyframeCut);
    setPreserveMetadata(values.preserveMetadata);
    setPreserveChapters(values.preserveChapters);
    setMovFastStart(values.movFastStart);
    setAvoidNegativeTs(values.avoidNegativeTs);
    recipeControls.setAutoExportExtraStreams(values.autoExportExtraStreams);
    setEnableOverwriteOutput(values.enableOverwriteOutput);
    setCutFileTemplate(values.cutFileTemplate);
    setCutMergedFileTemplate(values.cutMergedFileTemplate);
    onRecipeFormatChange(values.outFormat);
  ...
  return (
    ...
    >
      {outFormat != null && <ExportRecipeSelector loadError={recipeControls.exportRecipesLoadError} onRetryLoad={recipeControls.retryExportRecipesLoad} recipes={recipeControls.exportRecipes} onChange={recipeControls.setExportRecipes} onApply={applyRecipe} currentSettings={{ exportMode: effectiveExportMode, outFormat, keyframeCut, preserveMetadata, preserveChapters, movFastStart, avoidNegativeTs, autoExportExtraStreams: recipeControls.autoExportExtraStreams, enableOverwriteOutput, cutFileTemplate, cutMergedFileTemplate }} />}
