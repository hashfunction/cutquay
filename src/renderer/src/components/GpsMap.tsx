...
export default function GpsMap({ filePath, streamIndex }: {
  ...
  return (
    <div style={{ width: '80vw', height: '60vh' }}>
      <h2>GPS track metadata</h2>
      <p>Coordinates are read locally from the selected track.</p>
      <div style={{ overflow: 'auto', height: '80%' }}>
        <table><thead><tr><th>Point</th><th>Latitude</th><th>Longitude</th></tr></thead>
          <tbody>{points.map((point, i) => <tr key={point.index}><td>{i + 1}</td><td>{point.lat}</td><td>{point.lng}</td></tr>)}</tbody>
        </table>
      </div>
