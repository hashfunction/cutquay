...
function App() {
  ...
  const { showGenericDialog, genericDialog, closeGenericDialog, confirmDialog, openExportFinishedDialog, openCutFinishedDialog, openCleanupFilesDialog, openShiftSegmentsDialog, openDecimateDialog } = useDialog();
  ...
  const { beginReport, onExportOutcome, planExportOutputs, recordExportCommand, finishExportReport } = useExportReports();

  const appendLastCommandsLog = useCallback((command: string) => {
    setFfmpegCommandLog((old) => [...old, { command, time: new Date() }]);
  }, []);
  const appendFfmpegCommandLog = useCallback((args: string[]) => {
    recordExportCommand(args);
    appendLastCommandsLog(getFfCommandLine('ffmpeg', args));
  }, [appendLastCommandsLog, recordExportCommand]);
  ...
  const toggleSimpleMode = useCallback(() => setSimpleMode((v) => {
    ...
    return !v;
  }), [setSimpleMode, showNotification]);

  const effectiveExportMode = useMemo<ExportRecipeV1['exportMode']>(() => {
  ...
  const generateOverviewWaveform = useCallback(async () => {
    if (working) return;
    try {
  ...
  const togglePlay = useCallback(({ resetPlaybackRate, requestPlaybackMode }: { resetPlaybackRate?: boolean, requestPlaybackMode?: PlaybackMode } | undefined = {}) => {
    ...
    if (playbackModeRef.current != null) {
      ...
      if (!isSomeSegmentAtCursor) { // if a segment is already at cursor, don't do anything
        ...
        if (playbackModeRef.current === 'play-selected-segments' || playbackModeRef.current === 'loop-selected-segments') {
  ...
  const exportRecipeSnapshot = useMemo<ExportRecipeV1>(() => {
    const current = { version: 1 as const, id: 'current', name: 'Current settings', exportMode: effectiveExportMode, outFormat: fileFormat ?? 'matroska', keyframeCut, preserveMetadata, preserveChapters, movFastStart, avoidNegativeTs, autoExportExtraStreams, enableOverwriteOutput, cutFileTemplate: cutFileTemplateOrDefault, cutMergedFileTemplate: cutMergedFileTemplateOrDefault };
    const saved = allUserSettings.settings.exportRecipes.find((recipe) => Object.entries(current).every(([key, value]) => key === 'id' || key === 'name' || recipe[key as keyof ExportRecipeV1] === value));
    return saved ?? current;
  }, [allUserSettings.settings.exportRecipes, effectiveExportMode, fileFormat, keyframeCut, preserveMetadata, preserveChapters, movFastStart, avoidNegativeTs, autoExportExtraStreams, enableOverwriteOutput, cutFileTemplateOrDefault, cutMergedFileTemplateOrDefault]);

  const effectiveExportSettings = useMemo(() => ({
    mode: isEncoding ? (lossyMode ? 'lossy' : 'smart-cut') : 'lossless',
    outputPlaybackRate,
    ...
    paramsByFile: [...paramsByFile].map(([path, params]) => ({ path, ...params, paramsByStream: [...params.paramsByStream] })),
    copyFileStreams,
    ffmpegVersion: ffmpegInfo?.program_version.version,
    ffmpegConfiguration: ffmpegInfo?.program_version.configuration,
    customFfPath,
    encBitrate,
    lossyMode,
  }), [isEncoding, outputPlaybackRate, isRotationSet, effectiveRotation, shortestFlag, ffmpegExperimental, preserveMovData, preserveMetadataOnMerge, segmentsToChapters, cutFromAdjustmentFrames, cutToAdjustmentFrames, paramsByFile, copyFileStreams, ffmpegInfo, customFfPath, encBitrate]);
  ...
  const userConcatFiles = useCallback(async ({ paths, includeAllStreams, streams, fileFormat: outFormat, clearBatchFilesAfterConcat, generatedFileNames }: {
    ...
    try {
      ...
      const run = beginReport({ operation: 'merge', sources: paths, recipe: { ...exportRecipeSnapshot, exportMode: 'merge', outFormat }, segments: [], effective: { ...effectiveExportSettings, mode: 'stream-copy', streams, includeAllStreams } }, getOutDir(customOutDir, firstPath), warnings, notices);
      ...
      planExportOutputs([outPath]);
      const { haveExcludedStreams, outcome } = await concatFiles({ originalInputPaths: run.sources, paths, outPath, outDir, outFormat, metadataFromPath, includeAllStreams, streams, ffmpegExperimental, onProgress: setProgress, preserveMovData, movFastStart, preserveMetadataOnMerge, chapters: chaptersFromSegments });
      ...
      const reportPath = await finishExportReport();

      if (!hideAllNotifications) {
        showOsNotification(i18n.t('Merge finished'));
        openCutFinishedDialog({ filePath: outcome.status === 'created' ? outPath : reportPath ?? outPath, reportPath, notices: [...notices], warnings: [...warnings] });
      }
    } catch (err) {
      await finishExportReport(err);
      ...
      await finishExportReport();
      setWorking(undefined);
      setProgress(undefined);
    }
  }, [beginReport, exportRecipeSnapshot, effectiveExportSettings, planExportOutputs, finishExportReport, openCutFinishedDialog, workingRef, ensureWritableOutDir, customOutDir, setWorking, segmentsToChapters, concatFiles, ffmpegExperimental, preserveMovData, movFastStart, preserveMetadataOnMerge, closeBatch, hideAllNotifications, t, showOsNotification, handleConcatFailed]);
  ...
  const onExportConfirm = useCallback(async () => {
    ...
    const run = beginReport({ operation: 'cut',
      sources: [...new Set([filePath, ...copyFileStreams.map((input) => input.path)])],
      recipe: exportRecipeSnapshot,
      segments: segmentsToExport.map((segment) => ({ requestedStart: segment.start, requestedEnd: segment.end, keyframeCut })),
      effective: effectiveExportSettings,
    }, outputDir, warnings, notices);
    try {
      ...
      if (segmentsToChaptersOnly) {
        ...
        if (hasAnySegmentOverlap(sortedSegments)) {
          throw new UserFacingError(i18n.t('Make sure you have no overlapping segments.'));
        }
      ...
      planExportOutputs(cutFileNames.map((name) => getOutPath({ customOutDir: outputDir, filePath, fileName: name })));
      ...
      if (willMerge) {
        ...
        planExportOutputs([mergedOutFilePath]);
        if (outFiles.some((outcome) => outcome.status !== 'created')) throw new UserFacingError('Merge stopped because a segment was skipped. Existing files were preserved; choose new filenames or enable overwrite in Export options.');
        const mergedResult = await concatCutSegments({
          originalInputPaths: run.sources,
          customOutDir,
          outFormat: fileFormat,
        ...
        const createdOutFiles = outFiles.flatMap((f) => (f.status === 'created' ? [f.path] : []));
        if (autoDeleteMergedSegments && mergedResult.outcome.status === 'created') {
          await tryDeleteFiles(createdOutFiles);
          for (const path of createdOutFiles) {
            if (!(await mainApi.pathExists(path))) onExportOutcome({ path, status: 'deleted_after_merge' });
            else warnings.add(`Could not remove merged segment: ${path}`);
          }
        }
        if (mergedResult.outcome.status !== 'created') mergedOutFilePath = undefined;
      }
      ...
      if (exportExtraStreams) {
        try {
          ...
          await extractStreams({ originalInputPaths: run.sources, customOutDir, streams: nonCopiedExtraStreams });
      ...
      const exportedPaths = willMerge && mergedOutFilePath != null ? [mergedOutFilePath] : outFiles.filter((f) => f.status === 'created').map((f) => f.path);
      const reportPath = await finishExportReport();
      const revealPath = exportedPaths[0] ?? reportPath;
      if (!hideAllNotifications && revealPath != null) {
        showOsNotification(i18n.t('Export finished'));
        openCutFinishedDialog({ filePath: revealPath, reportPath, warnings: [...warnings], notices: [...notices] });
      }
  ...
  const captureSnapshotToClipboard = useCallback(async () => {
    ...
    try {
      ...
      await withErrorHandling(async () => {
        const currentTime = getRelevantTime();
        await captureFrameToClipboard({ filePath, time: currentTime, quality: captureFrameQuality });

        if (simpleMode && !prefersReducedMotion) shootConfetti();
        if (!hideAllNotifications) getSwal().toast.fire({ icon: 'info', title: t('Screenshot captured clipboard') });
      }, i18n.t('Failed to capture frame'));
    } finally {
      setWorking(undefined);
    }
  }, [captureFrameQuality, captureFrameToClipboard, filePath, getRelevantTime, hideAllNotifications, prefersReducedMotion, setWorking, simpleMode, t, withErrorHandling, workingRef]);
  ...
  }, [playingRef, setPlaybackRate, videoRef]);
  ...
  const extractAllStreams = useCallback(async () => {
    ...
    if (workingRef.current) return;
    const extractionStreams = mainCopiedStreams;
    const warnings = new Set<string>();
    const notices = new Set<string>();
    const run = beginReport({ operation: 'extract', sources: [filePath], recipe: exportRecipeSnapshot, segments: [], effective: { mode: 'stream-copy', streams: extractionStreams, recipeFieldsApplied: ['enableOverwriteOutput'], containerPolicy: 'Full-track extraction chooses the container by stream codec; muxers are recorded in commands.', ffmpegVersion: effectiveExportSettings.ffmpegVersion, customFfPath: effectiveExportSettings.customFfPath } }, getOutDir(customOutDir, filePath), warnings, notices);
    try {
      setWorking({ text: i18n.t('Extracting all streams') });
      setStreamsSelectorShown(false);
      const outcomes = await extractStreams({ originalInputPaths: run.sources, customOutDir, streams: extractionStreams });
      const reportPath = await finishExportReport();
      const firstExtractedPath = outcomes.find((outcome) => outcome.status === 'created')?.path ?? reportPath;
      if (!hideAllNotifications && firstExtractedPath != null) {
        showOsNotification(i18n.t('All tracks have been extracted'));
        openCutFinishedDialog({ filePath: firstExtractedPath, reportPath, warnings: [...warnings], notices: [...notices] });
      }
    } catch (err) {
      await finishExportReport(err);
      if (isAbortedError(err)) return;
  ...
  }, [beginReport, exportRecipeSnapshot, effectiveExportSettings, finishExportReport, openCutFinishedDialog, confirmDialog, customOutDir, extractStreams, filePath, hideAllNotifications, mainCopiedStreams, setWorking, showOsNotification, t, workingRef]);
  ...
  const extractSingleStream = useCallback(async (index: number) => {
    ...
    if (workingRef.current) return;
    const extractionStreams = mainStreams.filter((stream) => stream.index === index);
    const warnings = new Set<string>();
    const notices = new Set<string>();
    const run = beginReport({ operation: 'extract', sources: [filePath], recipe: exportRecipeSnapshot, segments: [], effective: { mode: 'stream-copy', streams: extractionStreams, recipeFieldsApplied: ['enableOverwriteOutput'], containerPolicy: 'Full-track extraction chooses the container by stream codec; muxers are recorded in commands.', ffmpegVersion: effectiveExportSettings.ffmpegVersion, customFfPath: effectiveExportSettings.customFfPath } }, getOutDir(customOutDir, filePath), warnings, notices);
    try {
  ...
  }, [beginReport, exportRecipeSnapshot, effectiveExportSettings, finishExportReport, openCutFinishedDialog, customOutDir, extractStreams, filePath, hideAllNotifications, mainStreams, setWorking, showOsNotification, workingRef]);
