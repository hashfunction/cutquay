...
import { exportOutput, isExportCancelled } from '../../../common/exportOperation';

import { getStdioString, getSuffixedOutPath, transferTimestamps, getOutFileExtension, getOutDir, getHtml5ifiedPath, unlinkWithRetry, getFrameDuration, isMac, html5ifiedPrefix, html5dummySuffix, assertFileExists } from '../util';
import { isCuttingStart, isCuttingEnd, runFfmpegWithProgress, getFfCommandLine, getDuration, createChaptersFromSegments, readFileFfprobeMeta, getExperimentalArgs, getVideoTimescaleArgs, logStdoutStderr, runFfmpegConcat, runFfmpeg } from '../ffmpeg';
...
async function writeChaptersFfmetadata(outDir: string, chapters: Chapter[] | undefined) {
  ...
  const path = join(outDir, `.cutquay-ffmetadata-${globalThis.crypto.randomUUID()}.txt`);

  const ffmetadata = chapters.map(({ start, end, name }) => (
    `[CHAPTER]\nTIMEBASE=1/1000\nSTART=${Math.floor(start * 1000)}\nEND=${Math.floor(end * 1000)}\ntitle=${name || ''}`
  )).join('\n\n');
  console.log('Writing chapters', ffmetadata);
  await writeFile(path, ffmetadata, { flag: 'wx' });
  return path;
}
...
function useFfmpegOperations({ filePath, treatInputFileModifiedTimeAsStart, treatOutputFileModifiedTimeAsStart, isEncoding, lossyMode, enableOverwriteOutput, outputPlaybackRate, cutFromAdjustmentFrames, cutToAdjustmentFrames, appendLastCommandsLog, encCustomBitrate, appendFfmpegCommandLog, ffmpegHwaccel, onExportOutcome, onExportPlanned }: {
  ...
  const concatFilesRaw = useCallback(async ({ paths, outDir, outPath, metadataFromPath, includeAllStreams, streams, outFormat, ffmpegExperimental, onProgress = () => undefined, preserveMovData, movFastStart, chapters, preserveMetadataOnMerge, videoTimebase }: {
    paths: string[],
  ...
  }, [appendLastCommandsLog, appendFfmpegCommandLog, treatInputFileModifiedTimeAsStart, treatOutputFileModifiedTimeAsStart]);

  const concatFiles = useCallback(async (options: Parameters<typeof concatFilesRaw>[0] & { originalInputPaths: readonly string[] }) => {
    let haveExcludedStreams = false;
    const outcome = await exportOutput({ io: mainApi,
      path: options.outPath,
      sources: [...options.originalInputPaths, ...options.paths, ...(options.metadataFromPath ? [options.metadataFromPath] : [])],
      overwrite: enableOverwriteOutput,
      onOutcome: onExportOutcome,
      run: async (stagingPath) => {
        const result = await concatFilesRaw({ ...options, outPath: stagingPath, outDir: dirname(stagingPath) });
        haveExcludedStreams = result.haveExcludedStreams;
      } });
    return { haveExcludedStreams, outcome };
  }, [concatFilesRaw, enableOverwriteOutput, onExportOutcome]);
  ...
  const cutMultiple = useCallback(async ({
    originalInputPaths, outputDir, segments: segmentsIn, cutFileNames, fileDuration, rotation, detectedFps, onProgress: onTotalProgress, keyframeCut, copyFileStreams, allFilesMeta, outFormat, shortestFlag, ffmpegExperimental, preserveMetadata, preserveMetadataOnMerge, preserveMovData, preserveChapters, movFastStart, avoidNegativeTs, paramsByFile, chapters,
  }: {
    originalInputPaths: readonly string[],
    outputDir: string,
    ...
    const cutSegment = async ({ start: desiredCutFrom, end: cutTo }: { start: number, end: number }, i: number, finalOutPath: string) => {
      ...
      const losslessPartOutPath = segmentNeedsSmartCut
        ? getSuffixedOutPath({ customOutDir: dirname(finalOutPath), filePath, nameSuffix: `smartcut-segment-copy-${i}${ext}` })
        : finalOutPath;
      ...
      const smartCutEncodedPartOutPath = getSuffixedOutPath({ customOutDir: dirname(finalOutPath), filePath, nameSuffix: `smartcut-segment-encode-${i}${ext}` });
      ...
      try {
        ...
        await concatFilesRaw({ paths: smartCutSegmentsToConcat, outDir: dirname(finalOutPath), outPath: finalOutPath, metadataFromPath: losslessPartOutPath, outFormat, includeAllStreams: true, streams: streamsAfterCut, ffmpegExperimental, preserveMovData, movFastStart, chapters, preserveMetadataOnMerge, videoTimebase, onProgress: onConcatProgress });
        return { path: finalOutPath, created: true };
      } finally {
    ...
    try {
      return await pMap(segments, async (segment, index) => {
        const path = join(outputDir, cutFileNames[index]!);
        return exportOutput({ io: mainApi, path, sources: [...originalInputPaths, filePath, ...copyFileStreams.map((input) => input.path)], overwrite: enableOverwriteOutput, onOutcome: onExportOutcome, run: (stagingPath) => cutSegment(segment, index, stagingPath) });
      }, { concurrency: 1 });
    } finally {
      if (chaptersPath) await tryDeleteFiles([chaptersPath]);
    }
  }, [shouldSkipExistingFile, isEncoding, filePath, lossyMode, losslessCutSingle, cutEncodeSmartPart, encCustomBitrate, concatFilesRaw, enableOverwriteOutput, onExportOutcome]);

  const concatCutSegments = useCallback(async ({ originalInputPaths, customOutDir, outFormat, segmentPaths, ffmpegExperimental, onProgress, preserveMovData, movFastStart, chapterNames, preserveMetadataOnMerge, mergedOutFilePath }: {
    customOutDir: string | undefined,
    outFormat: string | undefined,
    originalInputPaths: readonly string[],
    segmentPaths: string[],
    ...
    return concatFiles({ originalInputPaths, paths: segmentPaths, outDir, outPath: mergedOutFilePath, metadataFromPath, outFormat, includeAllStreams: true, streams, ffmpegExperimental, onProgress, preserveMovData, movFastStart, chapters, preserveMetadataOnMerge });
  ...
  // Plan all selected tracks before starting so later tracks remain visible in failed/cancelled reports.
  const extractStreams = useCallback(async ({ originalInputPaths, customOutDir, streams }: {
    originalInputPaths: readonly string[], customOutDir: string | undefined, streams: FFprobeStream[],
  }) => {
    invariant(filePath != null);
    const ordered = [...streams.filter((stream) => stream.codec_type !== 'attachment'), ...streams.filter((stream) => stream.codec_type === 'attachment')];
    const planned = ordered.map((stream) => {
      const attachment = stream.codec_type === 'attachment';
      const preferred = attachment ? undefined : getPreferredCodecFormat(stream);
      if (!attachment && preferred == null) throw new Error(`Unsupported stream ${stream.index}: ${stream.codec_type}`);
      const codec = attachment ? stream.codec_name : (stream.codec_name || stream.codec_tag_string || stream.codec_type);
      const extension = attachment ? (codec || 'bin') : preferred!.ext;
      const path = getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `stream-${stream.index}-${stream.codec_type}-${codec}.${extension}` });
      return { stream, attachment, preferred, path };
    });
    onExportPlanned(planned.map(({ path }) => path));
    return pMap(planned, async ({ stream, attachment, preferred, path }) => exportOutput({
      io: mainApi,
      path,
      sources: [...originalInputPaths, filePath],
      overwrite: enableOverwriteOutput,
      onOutcome: onExportOutcome,
      run: async (stagingPath) => {
        const args = attachment
          ? ['-y', '-hide_banner', '-loglevel', 'error', `-dump_attachment:${stream.index}`, stagingPath, '-i', filePath]
          : ['-hide_banner', '-i', filePath, '-map', `0:${stream.index}`, '-c', 'copy', '-f', preferred!.format, '-y', stagingPath];
        appendFfmpegCommandLog(args);
        try { await runFfmpeg(args); } catch (err) {
          // Dumping attachments reports this exit after writing. Publication independently
          // requires a nonempty file; cancellation and unrelated failures still reject.
          if (attachment && !isExportCancelled(err) && err instanceof Error && 'exitCode' in err && 'stderr' in err && err.exitCode === 1
            && (typeof err.stderr === 'string' || err.stderr instanceof Uint8Array)
            && getStdioString(err.stderr).trim() === 'At least one output file must be specified') return;
          throw err;
        }
      },
    }), { concurrency: 1 });
  }, [appendFfmpegCommandLog, enableOverwriteOutput, filePath, onExportOutcome, onExportPlanned]);
