...
import { parseExportRecipes, type ExportRecipeV1 } from '../../../common/exportRecipe';
...
export default function useUserSettingsRoot() {
  ...
  const loadExportRecipes = useCallback((): { recipes: ExportRecipeV1[], error: string | undefined } => {
    try {
      const raw = configStore.get('exportRecipes');
      // Clone remote objects before validating them in the renderer.
      // eslint-disable-next-line unicorn/prefer-structured-clone
      const value: unknown = raw === undefined ? undefined : JSON.parse(JSON.stringify(raw));
      return { recipes: parseExportRecipes(value), error: undefined };
    } catch (error) {
      console.error('Unable to load saved export recipes', error);
      return { recipes: [], error: 'Saved recipes could not be loaded. Restore your config.json backup, then retry loading. Recipe changes are disabled until loading succeeds.' };
    }
  }, []);
  const [recipeState, setRecipeState] = useState(loadExportRecipes);
  const recipeStateRef = useRef(recipeState);
  const exportRecipes = recipeState.recipes;
  const exportRecipesLoadError = recipeState.error;
  const retryExportRecipesLoad = useCallback(() => {
    const next = loadExportRecipes();
    recipeStateRef.current = next;
    setRecipeState(next);
  }, [loadExportRecipes]);
  const setExportRecipes = useCallback((action: SetStateAction<ExportRecipeV1[]>) => {
    const { current } = recipeStateRef;
    if (current.error !== undefined) return;
    try {
      const recipes = parseExportRecipes(typeof action === 'function' ? action(structuredClone(current.recipes)) : action);
      // Write before updating the view. Failed reads never become writable empty collections,
      // and failed writes retain the last successfully saved collection in both places.
      configStore.set('exportRecipes', recipes);
      const next = { recipes, error: undefined };
      recipeStateRef.current = next;
      setRecipeState(next);
    } catch (error) {
      console.error('Unable to save export recipes', error);
      errorToast('Export recipes could not be saved. Your previous collection has been retained.');
    }
  }, []);
  ...
  return {
    ...
    setExportRecipes,
    exportRecipesLoadError,
    retryExportRecipesLoad,
