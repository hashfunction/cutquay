...
test('real rendered media must be paused, decoded, exact-path, sufficiently large and unobscured',()=>{
  ...
  validateScene(view,'C:\\demo\\Spring.webm',390,464.141);
  for(const mutation of [v=>{v.width=784;},v=>{v.visibleToast=true;},v=>{v.video[0].paused=false;},v=>{v.video[0].paused='false';},v=>{v.video[0].src='file:///C:/other.webm';},v=>{v.video[0].currentTime=0;},v=>{v.video[0].videoWidth=0;}]){
    const changed=structuredClone(view);mutation(changed);assert.throws(()=>validateScene(changed,'C:\\demo\\Spring.webm',390,464.141));
  }
});
test('export evidence must bind actual trim, saved recipe, non-overwrite and output',()=>{
  ...
  const report={version:1,status:'succeeded',operation:'cut',sources:['C:\\demo\\Spring.webm'],effective:{mode:'lossless'},segments:[{requestedStart:376,requestedEnd:400,keyframeCut:true}],recipe,outputs:[{path:'C:\\demo\\Teaser.webm',status:'created',sizeBytes:123}]};
  validateExport(report,{exportRecipes:[recipe]},report.sources[0],123);
  for(const mutation of [r=>{r.segments[0].requestedEnd=5;},r=>{r.recipe.enableOverwriteOutput=true;},r=>{r.recipe.name='Original CI';},r=>{r.outputs[0].path=r.sources[0];},r=>{r.outputs[0].sizeBytes=1;}]){
