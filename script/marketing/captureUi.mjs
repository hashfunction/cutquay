...
function inspectCapture(){
  ...
  return {width:innerWidth,height:innerHeight,devicePixelRatio,visibleToast:[...document.querySelectorAll('.swal2-toast')].some(visible),
    thumbnails:[...document.querySelectorAll('img')].filter(e=>visible(e)&&e.complete&&e.naturalWidth>50&&e.src.startsWith('blob:')).length,
    video:[...document.querySelectorAll('video')].map(v=>({src:v.currentSrc||v.src,duration:v.duration,currentTime:v.currentTime,paused:v.paused,seeking:v.seeking,readyState:v.readyState,error:v.error?.message??null,videoWidth:v.videoWidth,videoHeight:v.videoHeight}))};
}

async function run(request){
  const socket=new WebSocket(validateEndpoint(request.endpoint,request.port));
  ...
  const result={schema_version:1,purpose:'marketing screenshots only',consumer_acceptance:false,phase:request.phase,nonce:request.nonce,
    capture_source_commit:request.captureSource,qualified_source_commit:request.packageSource,package_full_name:request.packageFullName,
    process_id:request.processId,steps,captured:false};
  try{
    ...
    if(request.phase==='export'){
      ...
      const reports=(await readdir(path.dirname(request.media))).filter(n=>n.endsWith('.cutquay-report.json'));assert.equal(reports.length,1);
  ...
  }catch(error){primary=error;result.error=error.stack??String(error);}
  finally{client.close();result.renderer_exceptions=client.exceptions;await writeNewJson(path.join(request.evidence,'ui-result.json'),result);}
